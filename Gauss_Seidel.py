...
gauss_seidel_bp = Blueprint("gauss_seidel", __name__)
MAX_ITER = 100
N = 3
...
def _iterar_gauss_seidel(A, b, x0, es):
    ...
    for it in range(1, MAX_ITER + 1):
        ...
        for i in range(N):
            if A[i, i] == 0:
                raise ValueError(f"El elemento diagonal a{i + 1}{i + 1} es cero; Gauss-Seidel no puede continuar.")
            suma = b[i]
            for j in range(N):
                if j != i:
                    suma -= A[i, j] * x[j]
        ...
        ea = 0.0 if it == 1 else _calcular_ea(x, x_anterior)
        filas.append((it, float(x[0]), float(x[1]), float(x[2]), ea))
...
def _guardar(ejercicio, filas):
    conn = _db()
    cur = conn.cursor()
    cur.execute("DELETE FROM metodo_gauss_seidel WHERE ejercicio = %s", (ejercicio,))
    for it, x1, x2, x3, ea in filas:
        cur.execute(
            """
            INSERT INTO metodo_gauss_seidel (ejercicio, iteracion, x1, x2, x3, ea)
            VALUES (%s, %s, %s, %s, %s, %s)
            """,
...
def _graficar(ejercicio, filas):
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title="Convergencia — Método de Gauss-Seidel",
    ...
    ruta = f"static/imagenes/gauss_seidel_{ejercicio}.html"
    pio.write_html(fig, file=ruta, auto_open=False)
    return "/" + ruta


@gauss_seidel_bp.route("/gauss-seidel", methods=["POST"])
def ejecutar_gauss_seidel():
    try:
        ...
        filas = _iterar_gauss_seidel(A, b, x0, es)
        ...
        return jsonify({
            "mensaje": f"Método de Gauss-Seidel guardado correctamente.{aviso}",
            "imagen": imagen,
...
@gauss_seidel_bp.route("/resultados-gauss-seidel")
def resultados_gauss_seidel():
    try:
        _asegurar_tabla()
        conn = _db()
        cur = conn.cursor()
...
@gauss_seidel_bp.route("/buscar_ejercicio_gauss_seidel/<int:ejercicio>", methods=["GET"])
def buscar_ejercicio_gauss_seidel(ejercicio):
    try:
        _asegurar_tabla()
        conn = _db()
...
@gauss_seidel_bp.route("/eliminar-gauss-seidel/<int:ejercicio>", methods=["DELETE"])
def eliminar_gauss_seidel(ejercicio):
    try:
        _asegurar_tabla()
        conn = _db()
        cur = conn.cursor()
...
@gauss_seidel_bp.route("/actualizar-gauss-seidel", methods=["POST"])
def actualizar_gauss_seidel():
    try:
        _asegurar_tabla()
        ejercicio = int(_campo_texto("ejercicio", "el número de ejercicio"))
        conn = _db()
        cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM metodo_gauss_seidel WHERE ejercicio = %s", (ejercicio,))
